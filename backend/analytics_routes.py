...
def active_users(days: int = 30, limit: int = 10, db: Session = Depends(database.get_db)):
    """
    直近days日での総合アクティビティ（投稿/返信/出品/各種コメント/各種まとめ投稿）ランキング。
    含めるテーブル:
    - 掲示板: BoardPost, BoardReply
    - マーケット: MarketItem, MarketItemComment
    - 授業まとめ: CourseSummary, CourseSummaryComment
    - サークルまとめ: CircleSummary, CircleSummaryComment
    管理者(master*@ac.jp)は除外。
    """
    ...
    # マーケット 出品
    market_post_counts = db.query(
        models.User.id.label('uid'),
        func.count(models.MarketItem.id).label('market_posts')
    ).join(models.MarketItem, models.MarketItem.author_id == models.User.id)
    market_post_counts = market_post_counts.filter(
        models.MarketItem.created_at >= since,
        or_(models.User.email == None, not_(models.User.email.like('master%@ac.jp')))
    ).group_by(models.User.id).subquery()

    # マーケット コメント
    market_reply_counts = None
    if hasattr(models, 'MarketItemComment'):
        market_reply_counts = db.query(
            models.User.id.label('uid'),
            func.count(models.MarketItemComment.id).label('market_replies')
        ).join(models.MarketItemComment, models.MarketItemComment.author_id == models.User.id)
        market_reply_counts = market_reply_counts.filter(
            models.MarketItemComment.created_at >= since,
            or_(models.User.email == None, not_(models.User.email.like('master%@ac.jp')))
        ).group_by(models.User.id).subquery()

    # 授業まとめ 投稿
    course_post_counts = None
    if hasattr(models, 'CourseSummary'):
        course_post_counts = db.query(
            models.User.id.label('uid'),
            func.count(models.CourseSummary.id).label('course_posts')
        ).join(models.CourseSummary, models.CourseSummary.author_id == models.User.id)
        course_post_counts = course_post_counts.filter(
            models.CourseSummary.created_at >= since,
            or_(models.User.email == None, not_(models.User.email.like('master%@ac.jp')))
        ).group_by(models.User.id).subquery()

    # 授業まとめ コメント
    course_reply_counts = None
    if hasattr(models, 'CourseSummaryComment'):
        course_reply_counts = db.query(
            models.User.id.label('uid'),
            func.count(models.CourseSummaryComment.id).label('course_replies')
        ).join(models.CourseSummaryComment, models.CourseSummaryComment.author_id == models.User.id)
        course_reply_counts = course_reply_counts.filter(
            models.CourseSummaryComment.created_at >= since,
            or_(models.User.email == None, not_(models.User.email.like('master%@ac.jp')))
        ).group_by(models.User.id).subquery()

    # サークルまとめ 投稿
    circle_post_counts = None
    if hasattr(models, 'CircleSummary'):
        circle_post_counts = db.query(
            models.User.id.label('uid'),
            func.count(models.CircleSummary.id).label('circle_posts')
        ).join(models.CircleSummary, models.CircleSummary.author_id == models.User.id)
        circle_post_counts = circle_post_counts.filter(
            models.CircleSummary.created_at >= since,
            or_(models.User.email == None, not_(models.User.email.like('master%@ac.jp')))
        ).group_by(models.User.id).subquery()

    # サークルまとめ コメント
    circle_reply_counts = None
    if hasattr(models, 'CircleSummaryComment'):
        circle_reply_counts = db.query(
            models.User.id.label('uid'),
            func.count(models.CircleSummaryComment.id).label('circle_replies')
        ).join(models.CircleSummaryComment, models.CircleSummaryComment.author_id == models.User.id)
        circle_reply_counts = circle_reply_counts.filter(
            models.CircleSummaryComment.created_at >= since,
            or_(models.User.email == None, not_(models.User.email.like('master%@ac.jp')))
        ).group_by(models.User.id).subquery()
    ...
    q = db.query(
        ...
        func.coalesce(market_post_counts.c.market_posts, 0).label('market_posts'),
        func.coalesce((market_reply_counts.c.market_replies if market_reply_counts is not None else func.cast(0, models.Integer)), 0).label('market_replies'),
        func.coalesce((course_post_counts.c.course_posts if course_post_counts is not None else func.cast(0, models.Integer)), 0).label('course_posts'),
        func.coalesce((course_reply_counts.c.course_replies if course_reply_counts is not None else func.cast(0, models.Integer)), 0).label('course_replies'),
        func.coalesce((circle_post_counts.c.circle_posts if circle_post_counts is not None else func.cast(0, models.Integer)), 0).label('circle_posts'),
        func.coalesce((circle_reply_counts.c.circle_replies if circle_reply_counts is not None else func.cast(0, models.Integer)), 0).label('circle_replies'),
    )
    q = q.outerjoin(post_counts, post_counts.c.uid == models.User.id)
    q = q.outerjoin(reply_counts, reply_counts.c.uid == models.User.id)
    q = q.outerjoin(market_post_counts, market_post_counts.c.uid == models.User.id)
    if market_reply_counts is not None:
        q = q.outerjoin(market_reply_counts, market_reply_counts.c.uid == models.User.id)
    if course_post_counts is not None:
        q = q.outerjoin(course_post_counts, course_post_counts.c.uid == models.User.id)
    if course_reply_counts is not None:
        q = q.outerjoin(course_reply_counts, course_reply_counts.c.uid == models.User.id)
    if circle_post_counts is not None:
        q = q.outerjoin(circle_post_counts, circle_post_counts.c.uid == models.User.id)
    if circle_reply_counts is not None:
        q = q.outerjoin(circle_reply_counts, circle_reply_counts.c.uid == models.User.id)
    q = q.filter(or_(models.User.email == None, not_(models.User.email.like('master%@ac.jp'))))
    total_expr = (
        func.coalesce(post_counts.c.posts, 0) +
        func.coalesce(reply_counts.c.replies, 0) +
        func.coalesce(market_post_counts.c.market_posts, 0) +
        (market_reply_counts.c.market_replies if market_reply_counts is not None else 0) +
        (course_post_counts.c.course_posts if course_post_counts is not None else 0) +
        (course_reply_counts.c.course_replies if course_reply_counts is not None else 0) +
        (circle_post_counts.c.circle_posts if circle_post_counts is not None else 0) +
        (circle_reply_counts.c.circle_replies if circle_reply_counts is not None else 0)
    )
    ...
    for idx, r in enumerate(rows, start=1):
        total = int(
            (r.posts or 0) + (r.replies or 0) +
            getattr(r, 'market_posts', 0) + getattr(r, 'market_replies', 0) +
            getattr(r, 'course_posts', 0) + getattr(r, 'course_replies', 0) +
            getattr(r, 'circle_posts', 0) + getattr(r, 'circle_replies', 0)
        )
        if total <= 0:
            continue
        result.append({
