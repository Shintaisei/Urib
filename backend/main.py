...
async def run_migrations():
    ...
    try:
        ...
        # columns
        for col in ("grade_level", "grade_score", "difficulty_level", "reference_pdf", "university"):
            if not column_exists('course_summaries', col):
                if dialect == 'postgresql':
                    ...
                    elif col in ("grade_level","grade_score","difficulty_level"):
                        exec_tx(f"ALTER TABLE course_summaries ADD COLUMN IF NOT EXISTS {col} VARCHAR(20)", f"✅ {col}フィールドを追加しました")
                    elif col == "university":
                        exec_tx(f"ALTER TABLE course_summaries ADD COLUMN IF NOT EXISTS {col} VARCHAR(50)", f"✅ {col}フィールドを追加しました")
                else:
                    if col == "reference_pdf":
                        ...
                        exec_tx(f"ALTER TABLE course_summaries ADD COLUMN {col} VARCHAR(50)", f"✅ {col}フィールドを追加しました")
            else:
